...
var _test2 = __webpack_require__(6);
...
new _vue2.default({
	...
	methods: {
		add: function add(event) {
			//var mousePos = this.mousePos(canvas, event);
			//this.structures[this.config.currentStructure][this.config.currentPolygon].push({x: mousePos.x, y: mousePos.y});
			//this.render();
			(0, _jquery2.default)(".row").css("background-color", "green");
		},
		mousePos: function mousePos(canvas, event) {
		...
		render: function render() {
			...
			ctx.fillStyle = "#FFC107";
			ctx.strokeStyle = "#FFC107";
			for (var i = 0; i < this.structures[this.config.currentStructure].length; i++) {
				ctx.beginPath();
				var start = 0;
				if (this.config.draw != false && this.config.currentPolygon == i) {//Condición de inicio.
					//ctx.lineTo(x,y);
				}
				while (start < this.structures[this.config.currentStructure][i].length) {
					ctx.lineTo(this.structures[this.config.currentStructure][i][start].x, this.structures[this.config.currentStructure][i][start].y);
					console.log(this.structures[this.config.currentStructure][i][start].x);
					start++;
				}
				ctx.closePath();
				ctx.stroke();
				if (this.config.draw == false) {
					//Condición para cerrar la figura y pintarla.
